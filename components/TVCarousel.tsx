...
type Props = { title?: string; movies: TVSeries[]; isVertical?: boolean };

function TVCarousel({ title, movies, isVertical }: Props) {
  return (
    <div className="z-40">
      ...
      >
        {isVertical
          ? movies.map((movie) => (
              ...
              >
                <Link href={`/TVDetail/${movie.id}`}>
                  <TVCard movie={movie} />
                  <div className="max-w-2xl">
                    <p className="font-bold">
                      {movie.name} ({movie.release_date?.split("-")[0]})
                    ...
                    <SocialShare
                      shareUrl={`${process.env.SITE_BASE_URL}tvDetail/${movie?.id}`}
                      title={movie.name}
                    />
                  </div>
                </Link>
          ...
          : movies.map((movie) => (
              <Link key={movie.id} href={`/TVDetail/${movie.id}`}>
                <TVCard key={movie.id} movie={movie} isVertical />
              </Link>
            ))}
      </div>
    </div>
