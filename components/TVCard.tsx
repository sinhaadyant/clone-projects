...
const TVCard = ({ movie, isVertical }: Props) => {
  return (
    <div className="flex-shrink-0 relative cursor-pointer transform hover:scale-105 transition duration-300 ease-out hover:drop-shadow-lg">
      ...
      <Link key={movie.id} href={`/TVDetail/${movie.id}`}>
        {isVertical == false ? (
          <p className="absolute z-20 bottom-10 left-5">{movie.name}</p>
        ) : (
