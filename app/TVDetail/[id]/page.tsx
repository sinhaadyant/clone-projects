...
import { getRelatedTVSeries, getTVDetail } from "@/lib/getMovies";
...
const MovieDetail = async ({ params: { id } }: Props) => {
  ...
  const movie: any = await getTVDetail(id);
  const relatedSeries = await getRelatedTVSeries(id);
      ...
      <div className="flex flex-col space-y-5 mt-32 xl:mt-42">
        <div key={movie.id} className=" ">
          ...
          <div className="hidden lg:inline absolute mt-0 top-0 pt-40 xl:pt-52 left-0 lg:mt-40 bg-transparent z-20 h-full w-full bg-gradient-to-r from-gray-900/90 via-transparent to-transparent p-10 space-y-5 text-white ">
            ...
            <Button className="ml-1">{movie?.seasons.length} Seasons</Button>
            <div>
        ...
        <TVCarousel title="You may also like" movies={relatedSeries} />
      </div>
    </div>
  );
};
